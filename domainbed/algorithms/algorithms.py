...
import numpy as np
from domainbed.algorithms.utils import *
...
from domainbed.optimizers import get_optimizer, optim_SAM
...
from evil.evil import evilScheduler
...
class ERM_EVIL(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        super(ERM_EVIL, self).__init__(input_shape, num_classes, num_domains, hparams)

        self.register_buffer("update_count", torch.tensor([0]))
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = nn.Linear(self.featurizer.n_outputs, num_classes)
        self.discriminator = nn.Linear(self.featurizer.n_outputs, num_domains)
        ...
        self.domain_optimizer = get_optimizer(
            hparams["optimizer"],
            (list(self.featurizer.parameters()) + list(self.discriminator.parameters())),
            lr=self.hparams["lr"],
            weight_decay=self.hparams["weight_decay"],
        )

        self.pruner = evilScheduler(self.featurizer, self.optimizer, dense_allocation=hparams["dense_allocation"], alpha=hparams["alpha"], delta=hparams["delta"], static_topo=0, T_end=3000, ignore_linear_layers=False, grad_accumulation_n=hparams["grad_accumulation_n"])
        self.L, self.W = [], []
    ...
    def update(self, x, y, **kwargs):
        ...
        # optimizing invariant parameters
        self.optimizer.zero_grad()
        ...
        if self.update_count >= 1000:
            if self.update_count == 1000:
                self.pruner.apply_pruning(self.hparams["mask_initialization"])

            if self.pruner.if_keepmask() is False:
                # optimizing variant parameters
                self.domain_optimizer.zero_grad()
                all_z = self.featurizer(all_x)
                disc_out = self.discriminator(all_z) # EVIL
                disc_loss = F.cross_entropy(disc_out, disc_labels)
                disc_loss.backward()
                self.pruner()

        self.update_count += 1
        return {"loss": classifier_loss.item()}

    def predict(self, x):
        return self.classifier(self.featurizer(x))
    
    
class RigL(Algorithm):

    def __init__(self, input_shape, num_classes, num_domains, hparams):

        super(RigL, self).__init__(input_shape, num_classes, num_domains, hparams)
    ...
    def update(self, x, y, **kwargs):
        ...
        if self.update_count >= 1000:
            ...
            if self.pruner.if_keepmask() is False:
                ...
                self.domain_optimizer.zero_grad()
                all_z = self.featurizer(all_x)
                disc_out = self.classifier(all_z) # RigL
                disc_loss = F.cross_entropy(disc_out, all_y)
                disc_loss.backward()
                self.pruner()
...
class GRADVAR_EVIL(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        super(GRADVAR_EVIL, self).__init__(input_shape, num_classes, num_domains, hparams)
    ...
    def update(self, x, y, **kwargs):
        ...
        if self.update_count >= 1000:
            ...
            if self.pruner.if_keepmask() is False:
                ...
                self.domain_optimizer.zero_grad()
                all_z = self.featurizer(all_x)
                disc_out = self.discriminator(all_z)
                disc_loss = F.cross_entropy(disc_out, disc_labels)
                disc_loss.backward()
                self.pruner()
                ...
                for i, (x, y) in enumerate(minibatches):
                    ...
                    self.pruner.save_domain_wise_grad(i)
                    print('saving: ', i)
...
class SAVE_GRAD(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        super(SAVE_GRAD, self).__init__(input_shape, num_classes, num_domains, hparams)
        ...
        self.W = get_W(self.featurizer)

        self.class_grad_file = 'grads/class_grad_'
        self.domain_grad_file = 'grads/domain_grad_'
    ...
    def update(self, x, y, **kwargs):
        ...
        if (self.update_count-1) % 1000 == 0:
            save_grad = torch.zeros(1).cuda()
            for i, w in enumerate(self.W):
                # if sparsity is 0%, skip
                if i == 0:
                    continue
                save_grad = torch.cat([save_grad, w.view(-1)])
            torch.save(save_grad, self.class_grad_file+str(self.update_count.item()-1)+'.pt')
        # self.optimizer.step()
        ...
        if (self.update_count-1) % 1000 == 0:
            ...
            for i, w in enumerate(self.W):
                ...
                if i == 0:
                    continue
                save_grad = torch.cat([save_grad, w.view(-1)])
            torch.save(save_grad, self.domain_grad_file+str(self.update_count.item()-1)+'.pt')
...
class SAM_EVIL(Algorithm):

    def __init__(self, input_shape, num_classes, num_domains, hparams):

        super(SAM_EVIL, self).__init__(input_shape, num_classes, num_domains, hparams)
        ...
        self.id_list = [id(w) for w in self.pruner.W]
        self.params = [p for p in self.featurizer.parameters()]
        self.eps_mask = []
        for p in self.params:
            self.eps_mask.append(torch.zeros_like(p, requires_grad=False).cuda())
    ...
    def update(self, x, y, **kwargs):
        ...
        # 1. eps(w) = rho * g(w) / g(w).norm(2)
        #           = (rho / g(w).norm(2)) * g(w)
        grad_w = autograd.grad(loss, self.featurizer.parameters())
        scale = self.hparams["rho"] / self.norm(grad_w)
        eps = [g * scale * m for (g, m) in zip(grad_w, self.eps_mask)]

        # 2. w' = w + eps(w)
        with torch.no_grad():
            for p, v in zip(self.featurizer.parameters(), eps):
                p.add_(v)
        ...
        self.optimizer.zero_grad()
        loss.backward()
        # restore original featurizer params
        with torch.no_grad():
            for p, v in zip(self.featurizer.parameters(), eps):
                p.sub_(v)
        self.optimizer.step()
        ...
        if self.update_count >= 1000:
            ...
            if self.pruner.if_keepmask() is False:
                ...
                self.pruner()
                # update eps_mask
                if self.pruner.backward_masks is not None:
                    for i, p in enumerate(self.params):
                        if id(p) in self.id_list and self.pruner.backward_masks[self.id_list.index(id(p))] is not None:
                            self.eps_mask[i] = self.pruner.backward_masks[self.id_list.index(id(p))].reshape(p.shape)
                        else:
                            self.eps_mask[i] = torch.zeros_like(p, requires_grad=False).cuda()
...
class IRM_EVIL(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        super(IRM_EVIL, self).__init__(input_shape, num_classes, num_domains, hparams)
...
class VREx_EVIL(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        super(VREx_EVIL, self).__init__(input_shape, num_classes, num_domains, hparams)
...
class GroupDRO_EVIL(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        super(GroupDRO_EVIL, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.register_buffer("q", torch.Tensor())
...
class CORAL_EVIL(Algorithm):

    def __init__(self, input_shape, num_classes, num_domains, hparams, gaussian=False):
        super(CORAL_EVIL, self).__init__(input_shape, num_classes, num_domains, hparams)
        if gaussian:
            self.kernel_type = "gaussian"
        else:
            self.kernel_type = "mean_cov"
