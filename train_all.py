...
def main():
    ...
    parser.add_argument("--tuning_parameter", type=str, default=None, choices=["dense_allocation", "alpha", "delta", "grad_accumulation_n"])
    parser.add_argument("--tuning_value", type=float, default=None)
    parser.add_argument("--initialization_type", type=str, default=None, choices=["weight", "fisher", "random", "sensitivity"])
    parser.add_argument("--tuning_depth", type=float, default=None)
    ...
    traverse_dict = {}
    traverse_dict["dense_allocation"] = [0.1, 0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9] 
    traverse_dict["alpha"] = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4] 
    traverse_dict["delta"] = [100, 200, 300, 400, 500]
    traverse_dict["grad_accumulation_n"] =[1, 5, 10, 15, 20]

    tuning_param = args.tuning_parameter
    tune_val = args.tuning_value
    hparams["batch_size"] = 16


    # for tune_val in traverse_dict[tuning_param]:
    if tuning_param is not None:
        hparams[tuning_param] = tune_val

    if args.initialization_type is not None:
        hparams['mask_initialization'] = args.initialization_type

    if args.tuning_depth is not None:
        hparams['tuning_depth'] = args.tuning_depth
